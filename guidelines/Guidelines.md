...
- 显示当前连胜天数和完成率
- 展示今日群组动态预览
- 统计当日完成各目标所花时间，并通过饼图展示当日打卡时间分布（总时长 + 各目标占比）
...
- 创建新目标（标题、描述、图标、目标天数、**每次完成预计用时（小时+分钟）**、分类）
- 查看所有目标列表
- 编辑和删除目标
- 查看每个目标的历史打卡记录
- 目标分类：健康、学习、工作、生活、通用
...
- 三种消息类型：
  ...
  - 系统消息：加入、退出、活动通知
- 头像可点击跳转到用户主页
- 消息时间戳：今天显示具体时间；昨天显示“昨天+时间”；更早的消息显示“X天前+时间”或具体日期+时间
...
- 活动状态：即将开始、进行中、已结束
- 参与/退出活动：
  - 活动创建者创建后**不会自动参加活动**，需要和其他组员一样手动点击“参加活动”
  - 任意组员在活动卡片中点击“参加活动”后：
    - 在「我的目标」中自动生成一个以该活动为主题的目标（使用活动标题、描述和群组头像，目标天数为活动起止日期跨度）
    - 页面会跳转到目标页面，让用户立刻看到新创建的目标并可以进行打卡
    - 活动排行榜仅统计已参加活动的成员在活动期间的完成情况
- 实时排行榜（仅参与活动的成员会被统计）
- 奖励设置
...
- 评论功能
- 时间线展示
- 时间显示：一天内使用“刚刚 / X分钟前 / X小时前”，超过一天显示具体日期（如 3/5）
- 用户头像可跳转到用户主页
...
- 从个人中心右上角设置图标进入
- 修改昵称
- 更换或补充绑定邮箱（本地 Mock，将来可对接真实后端）
- 更换或补充绑定手机号（本地 Mock，将来可对接真实后端）
- 修改密码表单（当前为交互占位，不做实际校验与保存，仅重置表单）
- 退出登录（清除本地登录状态，回到游客模式）
...
- [x] 基于本地 Mock 的用户注册/登录与会话管理（支持邮箱/手机号+密码、手机号+验证码两种模式）
- [x] 游客模式：未登录时可浏览大部分页面但不展示个人数据，仅在 Profile/Settings 引导登录
- [x] 基于 AuthContext 的全局登录状态管理（本地 `localStorage` 持久化）
- [x] 个人资料本地编辑（昵称、邮箱、手机号）
- [ ] 接入真实后端认证（Supabase / 微信登录等）
- [ ] 路由保护（强制登录后的受限页面）
